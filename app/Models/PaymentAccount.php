...
class PaymentAccount extends Model
{
     ...
     * @return Money
     */
    public function parseMoney($amount)
    {
    ...
    public function getMinTransferableObject()
    {
        if (!isset($this->minTransferableObject)) {
            if (!$value = $this->supportedCurrency->min_amount) {
                $value = Money::USD(settings()->get('min_payment'), true);
                $amount = exchanger($value, new Currency($this->currency));
            } else {
                $amount = $this->parseMoney($value);
            }
            $this->minTransferableObject = $amount;
        }
    ...
    public function getMaxTransferableObject()
    {
        if (!isset($this->maxTransferableObject)) {
            if (!$value = $this->supportedCurrency->max_amount) {
                $value = Money::USD(settings()->get('max_payment'), true);
                $amount = exchanger($value, new Currency($this->currency));
            } else {
                $amount = $this->parseMoney($value);
            }
            $this->maxTransferableObject = $amount;
        }

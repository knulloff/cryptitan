...
class Coin extends Model
{
     ...
     * @return array
     */
    public function getMarketChart(string $interval)
    {
        if (!isset($this->marketChart)) {
            $key = "coin.{$this->identifier}.marketChart.{$interval}";
            $expires = now()->add($interval, 1);

            $this->marketChart = Cache::remember($key, $expires, function () use ($interval) {
                return $this->adapter->marketChart($interval);
            });
        }

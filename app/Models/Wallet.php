...
class Wallet extends Model
{
    ...
    protected static function booted()
    {
        static::retrieved(function (self $wallet) {
            if ($wallet->coin->getDollarPrice() <= 0) {
     ...
     * @return CoinFormatter
     */
    public function getPeerFee(CoinFormatter $amount, $category)
    {
     ...
     * @return Money
     */
    public function castMoney($amount, string $currency, bool $convert = false): Money
    {
        return new Money($amount, currency($currency, $this->coin->currency_precision), $convert);
    }
    ...
    {
        return $this->coin->getValueObject();
    }
     ...
     * @return Collection
     */
    public function marketChart(string $interval, string $currency)
    {
        return collect($this->coin->getMarketChart($interval))

...
class PaymentTransaction extends Model
{
    ...
    {
        return new Money($this->getAttributeFromArray('value'), new Currency($this->account->currency));
    }
    ...
    {
        return new Money($this->getAttributeFromArray('balance') ?: 0, new Currency($this->account->currency));
    }
    ...
    public function completeGateway()
    {
        return $this->acquireLock(function (self $record) {
            $feature = FeatureLimit::paymentsDeposit();
            ...
            if ($gateway->verify($record->gateway_ref)) {
                $feature->setUsage($record->getValueObject(), $record->account->user);

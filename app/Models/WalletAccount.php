...
class WalletAccount extends Model
{
     ...
     * @return float
     */
    public function getMinPriceMarginAttribute()
    {
        $multiplier = (100 - min(settings()->get('price_margin'), 99)) / 100;
        return $this->castPriceAsMoney()->multiply($multiplier)->getValue();
    }
     ...
     * @return float
     */
    public function getMaxPriceMarginAttribute()
    {
        $multiplier = (100 + min(settings()->get('price_margin'), 99)) / 100;
        return $this->castPriceAsMoney()->multiply($multiplier)->getValue();
    }
    ...
    public function getBalanceOnTrade()
    {
        if (!isset($this->balanceOnTradeAttribute)) {
            ...
            $this->balanceOnTradeAttribute = (string) BigDecimal::of($exchangeTrade)->plus($peerTrade);
        }
     ...
     * @return CoinFormatter
     */
    public function getBuyExchangeFee(CoinFormatter $amount)
    {
     ...
     * @return CoinFormatter
     */
    public function getSellExchangeFee(CoinFormatter $amount)
    {
     ...
     * @return Money
     */
    public function castPriceAsMoney(): Money
    {
        return $this->wallet->castMoney($this->price, $this->user->currency, true);
    }
    ...
    public function parseTarget($target)
    {
        if ($target instanceof self) {
            ...
            if (filter_var($target, FILTER_VALIDATE_EMAIL)) {
                ...
                if ($account->is($this)) {
                    throw new TransferException(trans('wallet.cannot_send_to_same_account'));
                } elseif ($account->wallet->isNot($this->wallet)) {
                    throw new TransferException(trans('wallet.different_account_parent_wallet'));
                }
    ...
    public function send($amount, $target)
    {
        return $this->acquireLock(function () use ($amount, $target) {
            ...
            $amount = $account->parseCoin($amount);
            ...
            if ($target instanceof self) {
                ...
                return tap($record, function (TransferRecord $record) use ($account, $target, $amount, $operator, $withdrawalFee, $coin) {
                    ...
                    } elseif ($resource instanceof PendingApproval) {
                    ...
                    if ($operator?->isNot($account) && $withdrawalFee->isPositive()) {
